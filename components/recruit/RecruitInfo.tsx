...
export default function RecruitInfo() {
  return (
    <section className="bg-[var(--color-navy)] py-[58px] md:py-[88px]">
      <div className="mx-auto max-w-[1600px] px-5 md:px-10">
        <div className="flex items-end justify-between flex-wrap gap-4">
          ...
          <Link
            href="/recruit"
            className="group inline-flex items-center gap-1 text-[12px] font-semibold tracking-[0.08em] text-white hover:underline"
          >
            採用情報一覧を見る
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-[21px] md:gap-[28px]">
          {recruitCards.map((card, i) => (
            <Reveal key={card.title} delay={i * 100}>
              <Link href={card.href} className="group block bg-white">
                <div className="relative aspect-[40/33] overflow-hidden">
                  <Image
                    src={card.image.src}
                    alt={card.image.alt}
                    fill
