        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-[var(--color-border)]">
          {numberItems.map((item, i) => (
            <Reveal
              key={item.label}
              delay={i * 100}
              className="py-8 sm:py-0 sm:px-8 first:pl-0 sm:first:pl-0"
            >
              <div className="text-[var(--color-navy)]">{icons[i]}</div>
              <p className="mt-4 text-sm text-[#555555]">{item.label}</p>
              <p className="font-display-ja mt-2 text-[42px] font-semibold tracking-[0.01em] text-[var(--color-bordeaux)] md:text-[58px]">
                {item.value}
              </p>
